...
@Service
public class ReRankerEnginer {
	
	private final Logger log = LoggerFactory.getLogger(ReRankerEnginer.class);
	...
	public Collection<DocumentDTO> search(String query) {
		...
		try {
			...
			Query wildCardQuery = qp.parse(query);
			topDocs = searcher.search(wildCardQuery, totalResults);
			for (ScoreDoc scDoc : topDocs.scoreDocs) {
				log.info("The doc matched is {}", scDoc.toString());
				result.add(convertDocTDocumentDTO(searcher.doc(scDoc.doc),scDoc.score));
			}
			qp.setFuzzyMinSim(.2f);
			Query fuzzyQuery = qp.parse(query+"~10");
			log.info("The query created is {}", fuzzyQuery);
			topDocs = searcher.search(fuzzyQuery, totalResults);
			for (ScoreDoc scDoc : topDocs.scoreDocs) {
				log.info("The doc matched is {}", scDoc.toString());
				result.add(convertDocTDocumentDTO(searcher.doc(scDoc.doc),scDoc.score));
			}
